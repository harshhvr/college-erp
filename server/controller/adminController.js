...
function dobAsPassword(
  ...
  const password = [
    ...
    dob.getFullYear().toString().padStart(4, "0"),
  ].join("");
...
module.exports = {
  addAdmin: async (req, res, next) => {
    try {
      ...
      const password = dobAsPassword(dob);
  ...
  getTotalAdmins: async (req, res, next) => {
    try {
      const totalAdmins = await Admin.countDocuments();
      res.status(200).json({ totalAdmins });
    } catch (err) {
      console.log("Error in getting totalAdmins", err.message);
    }
  },
};
